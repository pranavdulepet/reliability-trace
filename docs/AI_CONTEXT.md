...
ReliabilityGraph helps answer one question: should the user trust this answer under the evidence gathered for this run? The product must show observable evidence, not hidden chain-of-thought. Every assistant answer can produce a Reliability Evidence Graph that can be inspected and exported.

The primary UI is multi-turn Chat. Users connect at least one LLM provider in Settings, ask questions in a thread, and optionally attach files or paste URLs in the composer. Recent turns are passed to providers as conversation messages, older turns are compressed into a continuity summary, and prior thread attachments/URLs remain reusable evidence for follow-up questions. Normal chat attempts web evidence automatically when a search key is configured; there is no main-canvas search off switch. The answer streams first. Reliability appears only after the full audit finishes: inline/source citations, one final Reliability Score, a short reason for the score, why that risk matters for this question, and concrete follow-up prompts/actions to improve reliability. Do not put provider selection or source management on the main chat canvas.
...
- The Reliability Score is a benchmark-tuned 0-100 audit score for ranking answer risk under gathered evidence, not proof, a provider confidence score, or a calibrated probability unless validated on the target distribution. Source-required answers are evidence-first: claim support, contradictions, retrieval alignment, and source quality dominate the score; stability can lower trust but cannot rescue unsupported factual claims. Source-grounded summaries are stricter: partially supported or temporally over-broad claims cap the score. Linear weights live in `configs/reliability_score_weights.json`; safety caps remain explicit policy, not learned weights.
