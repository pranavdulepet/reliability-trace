...
- Deterministic claim support checks for simple support, missing support, numeric/date conflicts, and polarity contradictions, plus optional provider-backed structured evidence assessment with schema validation, retry, redaction, and deterministic fallback.
...
- Provider-backed structured assumption extraction across providers.
- Full trained NLI-style claim verification beyond deterministic/source-local checks and optional provider assessment.
...
- Add a trained NLI verifier or benchmark-validated entailment model behind the same claim-assessment interface.
